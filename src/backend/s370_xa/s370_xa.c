    ...
    switch (instr->op) {
        ...
        case ANVIL_OP_LOAD:
            ...
            if (instr->operands[0]->kind == ANVIL_VAL_GLOBAL) {
                char upper_name[64];
                s370_xa_uppercase(upper_name, instr->operands[0]->name, sizeof(upper_name));
        ...
        case ANVIL_OP_STORE:
            ...
            if (instr->operands[1]->kind == ANVIL_VAL_GLOBAL) {
                char upper_name[64];
                s370_xa_uppercase(upper_name, instr->operands[1]->name, sizeof(upper_name));
                s370_xa_emit_load_value(be, instr->operands[0], S370_XA_R2);
                                          ...
                                          char **output, size_t *len)
{
    ...
    if (mod->num_globals > 0) {
        ...
        for (anvil_global_t *g = mod->globals; g; g = g->next) {
            char upper_name[64];
            s370_xa_uppercase(upper_name, g->value->name, sizeof(upper_name));
